                ...
                <AnimatePresence>
                    {recentActivity.length > 0 ?
                        recentActivity.map((data, key) => (
                            <motion.div 
                                key={key}
                                className="bg-bg-tertiary rounded-xl p-4 flex flex-col gap-4"
                                initial={{ opacity: 0, y: -20 }}
                                animate={{ opacity: 1, y: 0 }}
                                transition={{ duration: 0.2}}
                                exit={{ opacity: 0, y: -20}}
                            >
                                <div className="flex justify-center pb-1 border-b border-accent-secondary/20">
                                    <span className="text-2xl font-bold">{data.consistency}%</span>
                                </div>
                                <div className="w-fit grid grid-cols-2 gap-x-12 gap-y-3 m-auto text-md">
                                    <div>
                                        <span className="text-accent-secondary">Mode:</span>
                                        <div className="font-medium">{data.mode}</div>
                                    </div>
                                    <div>
                                        <span className="text-accent-secondary">BPM:</span>
                                        <div className="font-medium">{data.bpm} BPM</div>
                                    </div>
                                    <div>
                                        <span className="text-accent-secondary">Duration:</span>
                                        <div className="font-medium">{data.duration}s</div>
                                    </div>
                                    <div>
                                        <span className="text-accent-secondary">Clicks:</span>
                                        <div className="font-medium">{data.clicks}</div>
                                    </div>
                                </div>
                                <div className="text-center pt-1 border-t border-accent-secondary/20">
                                    <span className="text-sm text-accent-secondary font-medium">
                                        {formatDistanceToNow(data.setDate)} ago
                                    </span>
                                </div>
                            </motion.div>
                        )) : 
                        <div className="bg-bg-tertiary rounded-xl p-8 text-center">
                            <span className="font-medium text-xl text-accent-secondary">
                                No Recent Activity
                            </span>
                        </div>
                    }
